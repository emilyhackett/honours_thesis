...
As a summary of the above two sections, \autoref{sensitivity_time} shows plots of the sensitivity against the relative processing time for the six strain estimation techniques. Looking at these, clearly the finite difference based approaches have a significant speed advantage over the other methods, without loosing any sensitivity. In fact, the unweighted smoothing with FD and the pre-filtered smoothing with FD show better sensitivity than other methods when lateral averaging is implemented also.
...
\begin{figure}[h!]\ContinuedFloat
	...
	\end{subfigure}
	\caption{The images on the left show the axial image resolution, and those on the right the lateral image resolution for the six different strain estimation techniques, at different strain and lateral smoothing filter FWHM resolutions. Regions that are red correspond to worse image resolution.}
	\label{imageres_figs}
\end{figure}

\autoref{imageres_figs} show some interesting trends. The phase offset algorithm with SG filtering performs the worst over both axial and lateral image resolution, although has some good lateral resolution for the mid-range strain filter FWHM. The weighted FD has a high lateral image FWHM for any strain filter above approximately 40$\mu$m, however the axial resolution performs extremely well in comparison to other techniques. The WLS strain filter performs poorly for axial image resolution, however significantly better in the lateral resolution, which contrasts to the other technique utilising phase unwrapping, suggesting the problem arises in the strain filtering process. Both techniques that utilise SG filtering have streak-like features in the lateral resolution at different strain filter FWHM values, and are relatively high. Both the unweighted FD and pre-filtered FD show the best overall image resolution, with very similar axial resolution patterns, that are better at low strain and lateral smoothing filter FWHM values, as is expected. The pre-filtered FD 

Taking these results in conjunction with those regarding processing time and sensitivity in the sections above, it is decided that the unweighted FD strain estimation algorithm provides optimal processing speed, sensitivity, and image resolution. In particular, using a strain filter FWHM of 40$\mu$m and a lateral smoothing filter FWHM of 20$\mu$m should provide good axial and image resolution

\begin{table}
	\begin{tabularx}{\textwidth}{Yrrc}
		\toprule
		& \textbf{UW WLS} & \textbf{UWFD} & \textbf{Improved?}\\
		\midrule 
		...
		\bottomrule
	\end{tabularx}
	\caption{Comparison of the previously standard UW WLS strain estimation technique and the optimised UWFD.}
