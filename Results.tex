...
The \ac{pffd} algorithm was implemented as an effort to remove 'ripples' introduced by high intensity pixels in the finite difference with weighted smoothing, which will be discussed later in the results (see \autoref{qualitative1}). Using the weighted complex phase difference data, a small 2D Gaussian filter is applied to smooth out high intensity 'speckle' pixels. This pre-filter is set at $20 \mu m$ resolution, approximately on the order of the speckle size. This action aims to counter the disproportionate influence of high intensity pixels. From here, the strain is estimated using \ac{fd}, and the resulting strain is smoothed using a 2D Gaussian filter to match the required strain and lateral smoothing filter resolution values. Since the pre-filter contributes to the fit and lateral resolutions, for any given fit resolution $\text{FR}_{total}$ and lateral resolution $\text{LR}_{total}$ supplied to the function as input parameters, the resulting Gaussian smoothing resolutions must take into account the contribution from the pre-filter as in \autoref{prefilter_res_FR} and \autoref{prefilter_res_LR} below.
...
\begin{table}[h!]
	...
	\begin{tabularx}{\textwidth}{YlYY}
		...
		\midrule
		\ac{wls} with Unwrapped Phase & \ac{uwwls} & Volume unwrapping algorithm & Sequential WLS \\
		\ac{sg} Filtering on Unwrapped Phase & \ac{uwsg} & Volume unwrapping algorithm & SG filter convolution \\
		\ac{sg} Filtering on Offset Phase & \ac{posg} & Phase offset & Sequential SG filter convolution \\
		\ac{fd} on Weighted Smoothing of Phase & \ac{wfd} & None & FD with (weighted) Gaussian smoothing \\
		\ac{fd} on Unweighted Smoothing of Phase & \ac{uwfd} & None & FD with (unweighted) Gaussian smoothing \\
		Pre-Filtered Phase with Smoothed \ac{fd} & \ac{pffd} & None & FD with (weighted then unweighted) Gaussian smoothing \\
		\bottomrule
	\end{tabularx}
...
\subsection{Qualitative Comparison}{qualitative1}
...
The relative processing time (with respect to the \ac{wls} with unwrapped phase) is shown instead of the actual, to try provide an estimate that is comparable across different machines.
